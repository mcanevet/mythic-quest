...
permission:
  ...
  bash:
    ...
    "git merge*": allow          # dana: apply approved merges to trunk (wt16: merge reviewer role)
    "git checkout*": allow       # dana: switch to trunk for merging (wt16: merge reviewer role)
    "git rebase*": allow         # dana: replay worker commits on updated main (wt16: merge reviewer role)
...
compile-check procedure, naming conventions, and engine-specific
criteria — it is the single source for all of those.
...
3. Compile check FIRST, before any merge — per the review-merge skill's
   validation procedure, against each worktree, and re-validate on
   merged main after every merge: a merge that compiles in every branch
   but breaks on main is exactly what this gate exists to catch.
4. All workers approved → merge each worktree branch into main:
   ...
   engine session is using — the runtime lock protects scene-mutation
   tools only, not branch switches, so verify no runtime session is
   active before checkout/merge (the orchestrator's merge-slot
   acquisition + verify-role check already guards this; if a session
   slips through, report BLOCKED and stop).
...
- **Compile**: no parse errors, missing dependencies, type mismatches
  (per the review-merge skill's validation procedure).
...
- Scene files serialize; scripts parallelize: if two workers touched
  the same scene, prefer the worker owning that entity.
- If two workers edited the same script, REJECT and re-dispatch with
  clearer file ownership rather than hand-merging.
- Project settings file: prefer the more fundamental change; reject
  overlapping autoload additions — those must be coordinated upstream
  (exact filename per engine convention; see the review-merge skill).
