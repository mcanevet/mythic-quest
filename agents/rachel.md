...
2. Verify each dev-loop child via the engine runtime (tools named by the engine plugin's skills — playtest, scenario-gauntlet) — run the game, simulate input, assert state. The functional gauntlet is a SCENARIO, not a probe script: start from the canonical template (init-project reference, functional-gauntlet-scenario.json), copy to `tests/scenarios/functional_gauntlet.json`, customize per its checklist (bounds, economy-counter rates, terminal states), and run it via the scenario-runner entry point from the playtest skill — one awaited report. Ad-hoc engine probes are for diagnosing a specific violation group, never the gauntlet itself (wt14: ~20 ad-hoc probe scripts instead of extending one scenario; 15 were near-identical).
